...
namespace Fgv.Ide.Corporativohotsite.Tests
{
    ...
    /// Provides methods to easily work with <see cref="CorporativohotsiteDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<CorporativohotsiteTestModule>
    {
        ...
        {
            void NormalizeDbContext(CorporativohotsiteDbContext context)
            {
        ...
        protected void UsingDbContext(Action<CorporativohotsiteDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CorporativohotsiteDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CorporativohotsiteDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CorporativohotsiteDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CorporativohotsiteDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CorporativohotsiteDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CorporativohotsiteDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CorporativohotsiteDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CorporativohotsiteDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CorporativohotsiteDbContext, Task<T>> func)
        {
            T result;
