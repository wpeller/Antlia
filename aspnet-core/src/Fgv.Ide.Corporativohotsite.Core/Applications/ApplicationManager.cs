...
{
    public class ApplicationManager : CorporativohotsiteDomainServiceBase, IApplicationManager
    {
        ...
        public virtual string ValidateTokenHotSite(string token, int _secondsToExpire)
        {
            var dictionary = ConvertTokenToDictionary(token);

            var _app = dictionary["app"];

            if (_app == "HotSite")
            {
                return ValidateToken(token, "RedefinicaoSenha", _secondsToExpire);                
            }
            else
            {
                return null;
        ...
        public virtual string ValidateTokenSSOHotSite(string token, int _secondsToExpire)
        {
            var dictionary = ConvertTokenToDictionary(token);
            ...
            if (_app == "HotSite")
            {
               return ValidateToken(token, "SSOGerenciarInscritos", _secondsToExpire);               
            }
            else
            {
                return null;
