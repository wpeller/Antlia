...
namespace Fgv.Ide.Corporativohotsite.Authorization
{
	public static class LogInManagerExtensions
	{
		...
		public static ApplicationTokenHotsiteOutput TokenIsValidEmailHotSite(string token, int _prazoExpiracao)
		{
			var applicationManager = IocManager.Instance.Resolve<ApplicationManager>();
			var tokenDecrypt = applicationManager.ValidateTokenHotSite(token, _prazoExpiracao);
			
			return JsonConvert.DeserializeObject<ApplicationTokenHotsiteOutput>(TrataStringTokenDecryptToJson(tokenDecrypt));
			...
			string tenancyName)
		{
			...
			try
			{
				...
				identity.AddClaim(new Claim(CorporativohotsiteConsts.ClaimSiga2UserName, tokenObject.data.UsuarioLogado));
				identity.AddClaim(new Claim(CorporativohotsiteConsts.ClaimSiga2Name, tokenObject.data.UsuarioLogado));
			...
			string tenancyName, string usuarioUserName, string usuarioNome)
		{
			...
			try
			{
				...
				var identity = (ClaimsIdentity)(await principalFactory.CreateAsync(user)).Identity;
				identity.AddClaim(new Claim(CorporativohotsiteConsts.ClaimSiga2UserName, usuarioUserName));
				identity.AddClaim(new Claim(CorporativohotsiteConsts.ClaimSiga2Name, usuarioNome));
		...
		public static ApplicationTokenHotsiteOutput TokenIsValidSSOGerenciarInscritosHotSite(string token, int _prazoExpiracao)
		{
		
			var applicationManager = IocManager.Instance.Resolve<ApplicationManager>();
			var tokenDecrypt = applicationManager.ValidateTokenSSOHotSite(token, _prazoExpiracao);
			return JsonConvert.DeserializeObject<ApplicationTokenHotsiteOutput>(TrataStringTokenDecryptToJson(tokenDecrypt));		

		}		
		public static string CreateTokenGerenciarInscritosHotSite(string usuario, string idHotSite)
		{
			var applicationManager = IocManager.Instance.Resolve<ApplicationManager>();
			var tokenEncripto = applicationManager.CreateToken("HotSite", "SSOGerenciarInscritos", usuario, idHotSite);

			return tokenEncripto;
		}
