...
namespace Fgv.Ide.Corporativohotsite.Authorization.Distributed
{
    public class DistributedAuthorizationProvider : AuthorizationProvider
    {
        ...
        {
            var currentNavs = _repositorio.GetAllList(x => x.Module == CorporativohotsiteConsts.ModuleName);
        ...
        {
            return new LocalizableString(name, CorporativohotsiteConsts.LocalizationSourceName);
        }
    }
}
