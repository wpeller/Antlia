...
namespace Fgv.Ide.Corporativohotsite.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : CorporativohotsiteServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("Fgv.Ide.Corporativohotsite", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
