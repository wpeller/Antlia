...
namespace Fgv.Ide.Corporativohotsite.Storage
{
    public class TempFileCacheManager : ITempFileCacheManager
    {
        ...
        public void SetDocumentFile(string token, byte[] document)
        {
	        _cacheManager.GetCache(TempFileCacheName).Set(token, document, new TimeSpan(0, 4, 0, 0)); //expire time is 4 hours
        }
