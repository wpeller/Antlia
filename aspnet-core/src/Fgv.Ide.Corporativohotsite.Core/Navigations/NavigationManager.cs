...
{
    public class NavigationManager : CorporativohotsiteDomainServiceBase, INavigationManager
    {
        ...
        private async Task CreateOrUpdateIfNecessary(Navigation navigation, List<Navigation> currentExistedNavigations)
        {
            if(navigation.Module.IsNullOrEmpty())
                navigation.Module = CorporativohotsiteConsts.ModuleName;
        ...
        {
            var currentNavs = await _repositorio.GetAllListAsync(x => x.Module == CorporativohotsiteConsts.ModuleName);
