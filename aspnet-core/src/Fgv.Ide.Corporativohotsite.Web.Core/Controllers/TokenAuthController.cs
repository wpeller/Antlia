...
using Fgv.Ide.Corporativohotsite.Web.Authentication.JwtBearer;
using Fgv.Ide.Corporativohotsite.Web.Authentication.TwoFactor;
using Fgv.Ide.Corporativohotsite.Web.Models.TokenAuth;
using Fgv.Ide.Corporativohotsite.Authorization.Impersonation;
using Fgv.Ide.Corporativohotsite.Identity;
using Fgv.Ide.Corporativohotsite.IdentityManager;
...
namespace Fgv.Ide.Corporativohotsite.Web.Controllers
{
    ...
    public class TokenAuthController : CorporativohotsiteControllerBase
    {
		...
		[HttpPost]
		[UnitOfWork]
		public async Task<ExternalAuthenticateResultModel> ExternalLoginToken(string token)
		{
			var tenancyName = GetTenancyNameOrNull();

			var result = await _logInManager.GetExternalLoginTokenAsync(token, tenancyName);
			var accessToken = CreateAccessToken(await CreateJwtClaims(result.Identity, result.User));
			switch (result.Result)
			{
				case AbpLoginResultType.Success:
					result.User.SetSignInToken();
				...
				default:
					throw _abpLoginResultTypeHelper.CreateExceptionForFailedLoginAttempt(result.Result, result.User != null ? result.User.UserName : L("ExternalUser"), tenancyName);
			}
		}

		[HttpPost]
		public GenericResultObject<ApplicationTokenHotsiteOutput> ExternalTokenHotSite(string token, string tipo) 
		{
			ApplicationTokenHotsiteOutput result = new ApplicationTokenHotsiteOutput();
			...
			try
            {
				...
				{
					case "SENHA":
                        result = LogInManagerExtensions.TokenIsValidEmailHotSite(token, 7200);
						break;
                    case "SSOGERENCIARINSCRITOS":
						result = LogInManagerExtensions.TokenIsValidSSOGerenciarInscritosHotSite(token, 7200);
						break;
					default: 
						Logger.Warn("Unhandled login fail reason: " + result);
						throw new Exception("Não foi possível determinar o tipo do token a ser gerado.");
				}								
			}
			catch (Exception ex)
            {
				return new GenericResultObject<ApplicationTokenHotsiteOutput>(null, false, ex.Message.ToString());
            }

			return new GenericResultObject<ApplicationTokenHotsiteOutput>(result, true, string.Empty);
		}

		private Task<IdentityManagerDto> ValidarEConverterToken(string token)
		{
			return Task.FromResult(_identityManager.ValidateToken(token));
		}

		private async Task<AbpLoginResult<Tenant, User>> ObterLoginResult(string tenancyName, IdentityManagerDto identityManagerComunicacao)
		{
			var retorno = new UsuarioValidadoSiga2Dto();

			if (!identityManagerComunicacao.Success)
			{
				var autenticacaoSiga = await _usuarioServicoAppService.Autenticar(new AutenticarInput { Usuario = identityManagerComunicacao.User, Senha = identityManagerComunicacao.Password });

				if (autenticacaoSiga.Status == "UsuarioInexistente")
					if (_userManager.UsuarioInternoENaoPadraoParaImpersonate(identityManagerComunicacao.User))
						return await GetLoginResultAsync(
							identityManagerComunicacao.User,
							identityManagerComunicacao.Password,
							GetTenancyNameOrNull()
						);

				if (autenticacaoSiga.Status != "Autenticado")
					throw _abpLoginResultTypeHelper.CreateExceptionForFailedLoginAttempt(
						new AbpLoginResult<Tenant, User>(AbpLoginResultType.InvalidUserNameOrEmailAddress).Result, retorno.UserName,
						tenancyName);

				retorno.UserName = autenticacaoSiga.CodigoExterno;
				retorno.Nome = autenticacaoSiga.Nome;
			}
			else
			{
				var autenticacaoSiga = await _usuarioServicoAppService.ObterUsuarioPorCodigoExterno(identityManagerComunicacao.User);
				retorno.Nome = autenticacaoSiga.Nome;
				retorno.UserName = identityManagerComunicacao.User;
			}

			return await _logInManager.GetIdentityManagerTokenAsync(tenancyName, retorno.UserName, retorno.Nome);
		}

		[HttpPost]
		[UnitOfWork]
		public async Task<ExternalAuthenticateResultModel> IdentityManagerLogin(string token)
		{
			var tenancyName = GetTenancyNameOrNull();
			var informacoesToken = await ValidarEConverterToken(token);

			var result = await ObterLoginResult(tenancyName, informacoesToken);
