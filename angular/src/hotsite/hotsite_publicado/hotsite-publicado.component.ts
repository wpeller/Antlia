...
@Component({
    selector: 'hotsite-publicado',
    templateUrl: './hotsite-publicado.component.html',
    styleUrls: ['./hotsite-publicado.component.css']
})
export class HotSitePublicadoComponent implements OnInit {
    ...
    getMenu(idMenu: number, nomeHotSite: string, usuarioLogado: boolean): void {
        ...
        this._hotSiteInCompanyService.buscarMenuPublicadoPorId(idMenu)
        ...
        this._hotSiteInCompanyService.buscarHotSiteInCompanyByNome(nomeHotSite)
            ...
            .subscribe(x => {
                if (x == null || x.id == null) {
                    ...
                    this.message.error('Hotsite não publicado.');
                }
                else if (x.excluido) {
                    this.message.error('Hotsite excluido.');
                }
                else {
    ...
    menuOnClick(obj: FgvMenuHorizontalItemMenu): void {
        ...
        this._hotSiteInCompanyService.buscarMenuPublicadoPorId(obj.identificador)
            .pipe(finalize(() => { this.hideLoading(); }))
            .subscribe(y => {
                if (y != null) {
                    this.setConteudo(y.titulo, y.texto);
